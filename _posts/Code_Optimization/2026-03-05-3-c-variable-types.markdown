...
이 경우: 
CPU는 32bit 연산 가능
메모리에서는 64bit씩 읽어올 수 있음
내부에서 나눠서 사용

int a,b;
연속된 메모리에 있다면
메모리에서 한번에 64비트를 읽을 수 있어서 a(32bit) + b(64bit)를 한번에 가져올 수 있음

 CPU는 먼저 a 사용 → 다음에 b 사용
(이미 캐시나 버퍼 안에 있어서 추가 메모리 접근이 없음)

⇒ 메모리 접근 횟수 줄고, 캐시 효율 좋아서 성능 빠름 | 예시 2
          CPU: 64비트
          버스: 32비트

이 경우 : 
CPU는 64bit 연산 가능
버스가 32bit라 두번 전송해야 함

⇒ 메모리에서 32bit씩 두 번 읽어야 함
⇒ 성능 저하 발생 |
| --- | --- |

| 32bit CPU,  데이터 버스폭 32bit (4byte)

double c; (8byte)
변수 c를 메모리에서 읽어올 때 두 번의 메모리 접근 필요
char d; (1byte)

**CPU가 byte load 지원 시 (ARM, x86)**
   `LDRB r0, [addr]`
   1byte만 읽음
   버스가 4byte 전송 해도 CPU는 내부적으로 1byte만 사용해서 마스킹 추가 비용 거의 없음
            ⇒ 효율성 크게 떨어지지 않음

**CPU가 byte load 지원 안 함**
   `LDR r0, [addr]`  
   `AND r0, #0xFF`
   버스에서 4byte 전송 하면 CPU는 읽어온 4byte 중 1byte의 데이터를 추려내는 마스킹 연산 추가 → 효율성 떨어짐
하지만 현대 CPU에선 거의 해당 안 됨. |
| --- |
